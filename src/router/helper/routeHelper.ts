...
function asyncImportRoute(routes: AppRouteRecordRaw[] | undefined) {
  ...
  if (!routes) return;
  routes.forEach((item) => {
    const {component, name, componentName} = item;
    const {children} = item;
    ...
    item.meta.title == '首页' && (item.meta.affix = true)
  ...
  routeList.forEach((route) => {
    ...
    if (route.component) {
      if ((route.component as string).toUpperCase() === 'LAYOUT') {
        ...
        route.name = `${route.name}Parent2222`;
        route.path = '';
...
function promoteRouteLevel(routeModule: AppRouteModule) {
  ...
  router = null;
  routeModule.keepAlive = true
  routeModule.meta.keepAlive = true
