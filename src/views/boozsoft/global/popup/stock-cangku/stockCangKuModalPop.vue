<template>
  ...
  >
    ...
    <div style="width: calc(100% - 265px); float: right;margin-top: 5px;">
...
async function cangkuChange(val) {
  ...
  if(val.target.value=='1'){
    ...
    // 级别仓库
    setColumns([
      {title: '仓库信息',dataIndex: 'ckNum'},
      {title: '档案级次',dataIndex: 'parentLevelNum'},
      {title: '档案名称',dataIndex: 'title'},
      {title: '档案说明',dataIndex: 'recordExplain'}
    ])
    // 不是独立仓库的
    await useRouteApi(findByCkNoIsDuliTree,{schemaName: database})('0')
    .then(data=>{
      treeData.value=data
      // data.forEach(a=>{
      //   treeData.value.push(
      //     {title: a.ckName,key:a.id,value:a,type:'jibie',children: []}
      //   )
      // })
    })
    if (theData.value.ckIsDuli != null && theData.value.ckIsDuli == '0' && theData.value.entity != null){
      selectedKeys2.value=[theData.value.entity.id]
      findByStockCangKuLevel(theData.value.entity)
    }
  }
}
