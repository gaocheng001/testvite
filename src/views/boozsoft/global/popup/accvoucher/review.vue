<template>
  <BasicModal
    width="650px"
    v-bind="$attrs"
    :title="pageMark?'审核凭证':'弃审凭证'"
    :ok-text="pageMark?'审核':'弃审'"
    @ok="handleOk()"
    ...
    <a-spin :spinning="spinning" :delay="500">
      <div class="nc-open-content">
        <div style="width: 120px;border-bottom: 3px solid #0188e8">
          <span style="font-size: 18px;color: #2828c9;margin-left: 5px">{{pageMark?'审核':'弃审'}}范围</span>
        </div>
        <div style="margin: 4% 0">
          <a-radio-group v-model:value="pageVariable.scopeCondition" @change="radioChange">
            <a-radio :value="1">
              {{pageMark?'审核':'弃审'}}当前选择的凭证
            </a-radio>
            <div style="height: 10px"></div>
            <a-radio :value="2">
              按条件批量{{pageMark?'审核':'弃审'}}凭证
            </a-radio>
          </a-radio-group>
        </div>
        <div style="position: relative;border: 1px #666565 solid">
          <span style="position: inherit; top: -12px; left: 35px;background-color: white;">&emsp;条件批量{{pageMark?'审核':'弃审'}}&emsp;</span>
          <div>
            <ul>
              <li>
                <span>凭证期间：</span>
              ...
              <li>
                ...
                <a-input v-model:value="pageVariable.batchCondition.voucherNumberStart" :allowClear="true"
                         :disabled="pageVariable.scopeCondition == 1" placeholder="" style="width: 160px"/>
                <span>&emsp;~&emsp;</span>
              ...
              <li>
                ...
                <a-select v-model:value="pageVariable.batchCondition.cashier"
                          :disabled="pageVariable.scopeCondition == 1" style="width: 112px">
                </a-select>
              </li>
              <li v-if="!pageMark">
                <span>&emsp;审核人：</span>
...
const handleOk = async () => {
  ...
  if (pageVariable.scopeCondition === 1 && selectedRowKeys.value.length == 0) {
    ...
    message.info('批量审核凭证期间or凭证日期不能为空！')
    return false
  } else {
    spinning.value = true
    ...
    if (await checkMeetSpecifyConditions()) { // 开始审核
      ...
      let res = ''
    /*  if (pageMark.value){*/
        //res = await startVoucherReviewProcedural(data1)
    /*  }else {
    // 已失效
        res =  await startVoucherCloseReviewProcedural(data1)
      }*/
...
const checkMeetSpecifyConditions = async () => {
  ...
  if (closeMonth.length > 0) {
    createWarningModal({title: '开始审批前检测', content: '凭证' + closeMonth.toString() + '月份已经结账，不能进行凭证审核操作！'})
    //开始
    spinning.value = true
    changeOkLoading(spinning.value)
    return false
  ...
  for (let i=0;i<res.length;i++)
  {
    ...
    if (checkMenu.indexOf(item.functionModule) != -1 && item.state == '1') {
      ...
      }else if (!pageMark.value && item.functionModule == '凭证记账' && item.time != '' && !compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
      ...
      } else if (pageMark.value && item.functionModule == '审核凭证' && item.time != '' && !compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
      ...
      }else if (!pageMark.value && item.functionModule == '主管签字' && item.time != '' && !compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
