    ...
    <a-spin :spinning="spinning" :delay="500">
      <div class="nc-open-content">
        <div style="width: 120px;border-bottom: 3px solid #0188e8">
          <span style="font-size: 18px;color: #2828c9;margin-left: 5px">{{pageMark?'签字':'取签'}}范围</span>
        </div>
        <div style="margin: 4% 0">
          <a-radio-group v-model:value="pageVariable.scopeCondition" @change="radioChange">
            <a-radio :value="1">
              对当前选择的凭证进行{{pageMark?'签字':'取签'}}
            </a-radio>
            <div style="height: 10px"></div>
            <a-radio :value="2">
              按条件批量对凭证进行{{pageMark?'签字':'取签'}}
            </a-radio>
          </a-radio-group>
        </div>
        <div style="position: relative;border: 1px #666565 solid">
          <span style="position: inherit; top: -12px; left: 35px;background-color: white;">&emsp;条件批量{{pageMark?'签字':'取签'}}&emsp;</span>
          <div>
            <ul>
              <li>
                <span>凭证期间：</span>
...
import {startVoucherSignProcedural,startVoucherCloseSignProcedural} from '/@/api/record/system/accvoucher'
...
const handleOk = async () => {
  ...
  if (pageVariable.scopeCondition === 1 && selectedRowKeys.value.length == 0) {
    ...
    message.info('批量签字凭证期间or凭证日期不能为空！')
    return false
  } else {
    spinning.value = true
    ...
    if (await checkMeetSpecifyConditions()) { // 开始审核
      ...
      let res = null
      if (pageMark.value){
        res = await startVoucherSignProcedural(data1)
      }else {
        res = await startVoucherCloseSignProcedural(data1)
      }
...
const checkMeetSpecifyConditions = async () => {
  ...
  for (let i=0;i<res.length;i++)
  {
    ...
    if (checkMenu.indexOf(item.functionModule) != -1 && item.state == '1') {
      ...
      if (item.functionModule == '月末结账' && item.time != '' && !compareTime(offsetToStr(item.time))
        ...
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
          '】正在进行' + item.imonth + '月份' + item.functionModule + '处理!不能进行凭证主管签字操作，请销后再试，或联系财务主管清理该结账任务后再进行相关操作!'
        break;
      ...
      } else if (item.functionModule == '凭证记账' && item.time != '' && !compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
          '】正在进行' + item.functionModule + '处理!不能继续进行主管签字操作，该操作不能多人同时并发进行，请销后再试，或联系财务主管清理该审核凭证任务后再进行操作!'
        break;
      } else if (item.functionModule == '凭证记账' && item.time != '' && compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：发现操作员【' +
          item.caozuoUnique +
  ...
  if (msg != '') {
    createWarningModal({title: '开始签字前检测', content: msg})
    return false
  }
  return true;
}
