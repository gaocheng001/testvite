      ...
      <div class="open-content-up" style="height: 400px;margin-left: 5%;width:90%;">
        <div class="open-content-up-one">
          ...
          <div>
            ...
            <DownloadOutlined  style="font-size: 28px;margin: 0 2%" @click="downLoad"/><a @click="downLoad">模板下载</a>
            <br/>
            <label>参照格式：</label>
            <Select
              ...
              style="width: 50%;margin-right: 10%;"
            >
              <SelectOption  value="2">辅助核算项按编码导入</SelectOption>
              <SelectOption  value="1">辅助核算项按名称导入</SelectOption>
            </Select>
          </div>
        </div>
...
<!--    <div class="import-centent-div" style="margin: 2% 5% 0%">
      &lt;!&ndash;      <div style="position: relative;border:1px solid #9b9b9b;padding: 20px;">
              <span
...
const [register, {closeModal}] = useModalInner((data) => {
  ...
  findAllAccvoucherTemplate().then(res => {
    ...
    if (templateList.value.length >0)templateName.value = templateList.value[0].id+'--'+templateList.value[0].templateType+'--0'
  })
  fileList.value = []
})
          ...
          .then(async (info: any) => {
            ...
            if (null != info.pass) { // 检查
              ...
              if (info[0].code == 404) {
                ...
                message.success('导入成功')
                emit('reloadProjects')
                emit('log',info[0].result?.length || 0)
                closeModal()
              }
            }
            return false;
