    ...
    <a-spin :spinning="spinning" :delay="500">
      <div class="nc-open-content">
        <div style="width: 120px;border-bottom: 3px solid #0188e8">
          <span style="font-size: 18px;color: #2828c9;margin-left: 5px">{{pageMark?'记账':'取账'}}范围</span>
        </div>
        <div style="margin: 4% 0">
          <a-radio-group v-model:value="pageVariable.scopeCondition" @change="radioChange">
            <a-radio :value="1">
              对当前选择的凭证进行{{pageMark?'记账':'取账'}}
            </a-radio>
            <div style="height: 10px"></div>
            <a-radio :value="2">
              按条件批量对凭证进行{{pageMark?'记账':'取账'}}
            </a-radio>
          </a-radio-group>
        </div>
        <div style="position: relative;border: 1px #666565 solid">
          <span style="position: inherit; top: -12px; left: 35px;background-color: white;">&emsp;条件批量{{pageMark?'记账':'取账'}}&emsp;</span>
          <div>
            <ul>
              <li>
                <span>凭证期间：</span>
              ...
              <li>
                <span>&emsp;审核人：</span>
                <a-select v-model:value="pageVariable.batchCondition.checkName" :disabled="pageVariable.scopeCondition == 1"
                          style="width: 227px">
                </a-select>
              </li>
            </ul>
...
import {startVoucherBookProcedural,startVoucherCloseBookProcedural} from '/@/api/record/system/accvoucher'
...
const handleOk = async () => {
  ...
  if (pageVariable.scopeCondition === 1 && selectedRowKeys.value.length == 0) {
    ...
    if (await checkMeetSpecifyConditions()) { // 开始审核
      ...
      let res = null
      if (pageMark.value){
        res = await startVoucherBookProcedural(data1)
      }else {
        res = await startVoucherCloseBookProcedural(data1)
      }
...
const checkMeetSpecifyConditions = async () => {
  ...
  if (closeMonth.length > 0) {
    createWarningModal({title: '开始记账前检测', content: '凭证' + closeMonth.toString() + '月份已经结账，不能进行凭证记账操作！'})
    //开始
    spinning.value = true
    changeOkLoading(spinning.value)
    return false
  ...
  for (let i=0;i<res.length;i++)
  {
    ...
    if (checkMenu.indexOf(item.functionModule) != -1 && item.state == '1') {
      ...
      if (item.functionModule == '月末结账' && item.time != '' && !compareTime(offsetToStr(item.time))
        ...
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
          '】正在进行' + item.imonth + '月份' + item.functionModule + '处理!不能进行凭证记账操作，请销后再试，或联系财务主管清理该结账任务后再进行相关操作!'
        break;
      }else if ( item.time != '' && !compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
          '】正在进行' + item.functionModule + '处理!不能继续进行凭证记账操作，该操作不能多人同时并发进行，请销后再试，或联系财务主管清理该' + item.functionModule + '任务后再进行操作!'
        break;
      }
      if (!pageMark.value && item.functionModule == '月末结账' && item.time != '' && compareTime(offsetToStr(item.time))) {
        msg = '提示：发现操作员【' +
          item.caozuoUnique +
      ...
      }else if (!pageMark.value && item.functionModule == '凭证记账' && item.time != '' && compareTime(offsetToStr(item.time))) {
        msg = '提示：发现操作员【' +
          item.caozuoUnique +
          '】正在进行当前月份' + item.functionModule + '操作已经超过30分钟!可能发生任务异常导致任务超时，是否强制清除凭证记账？'
        if (await askTask({msg: msg, taskId: item.id, year: yearMonthsObj.year})){
          msg = ''
          continue
        }else {
  ...
  if (msg != '') {
    createWarningModal({title: '开始记账前检测', content: msg})
    return false
  }
  return true;
}
