  ...
  >
    <div class="nc-open-content" style="margin-top: 10px;margin-bottom: 10px;height: 240px;">
      <div class="open-content-up">
        <label>样式编码：</label>
        <Input v-model:value="styleHead.styleCode" @blur="checkName()" placeholder="请输入样式编码" style="width:15%"/>
        <label>样式名称：</label>
        <Input v-model:value="styleHead.styleName" placeholder="请输入样式名称" style="width:20%"/>
        <label>说明：</label>
        <Input v-model:value="styleHead.styleName" placeholder="请输入说明" style="width:20%"/>
        <br/><br/>
        <div>
        ...
        <table class="layui-table" id="tableId" style="height: calc( 100% - 20px );margin-top: 10px;background-color:#f2f4f6;">
          <thead>
            <tr class="dy_table_thead_tr even">
              ...
              <th style="width: 20%;">名称长度（%）</th>
              <th style="width: 20%;">输入框长度（%）</th>
          ...
          <tbody>
            <tr class="odd" v-for="(row,rowIndex) in formItems">
              <td style="width: 20%;">
                <Input v-model:value="row.cname" placeholder="请输入栏目名称" style="width:100%"/>
              </td>
              <td style="width: 20%;">
                <InputNumber v-model:value="row.nameLength" placeholder="取值范围在20～40" class="right" :precision="2" style="width: 100%;" :formatter="value => `${value}%`" :parser="value => value.replace('%', '')"/>
              </td>
              <td style="width: 20%;">
                <InputNumber v-model:value="row.inputLength" placeholder="取值范围在60～80" class="right" :precision="2" style="width: 100%;" :formatter="value => `${value}%`" :parser="value => value.replace('%', '')"/>
              </td>
              <td style="width: 20%;">
                <Input v-model:value="row.isShow" class="right" style="width: 100%;" />
              </td>
              <td style="width: 20%;">
