...
import {
  deleteByManageCodeAndIyearAndImonth,
  findByManageCodeAndIyearAndImonth,
  findDeptByManageCodeAndIyearAndImonth,
  findPeriodByUniqueCode, findProjectByManageCodeAndIyearAndImonth, saveFaDepreciationDeptList,
  saveFaDepreciationList, saveFaDepreciationProjectList,
  saveFaPeriod
...
const depreciationDeptList:any = ref([])
const depreciationProjectList:any = ref([])
async function handleOk() {
  ...
  //保存部门折旧表
  await useRouteApi(saveFaDepreciationDeptList,{schemaName: dynamicTenantId})(depreciationDeptList.value)
  //保存项目折旧表
  await useRouteApi(saveFaDepreciationProjectList,{schemaName: dynamicTenantId})(depreciationProjectList.value)
  //修改期间表折旧状态
...
async function calculationJitizhejiu() {
  ...
  depreciationDeptList.value = []
  depreciationProjectList.value = []
  ...
  const shangyueZhejiuDeptList: any = await useRouteApi(findDeptByManageCodeAndIyearAndImonth, {schemaName: dynamicTenantId})({code:formItems.value.manageCode,year:year1,month:month1})
  const shangyueZhejiuProjectList: any = await useRouteApi(findProjectByManageCodeAndIyearAndImonth, {schemaName: dynamicTenantId})({code:formItems.value.manageCode,year:year1,month:month1})
    ...
    if (yuezhejiue>0) {
      ...
      if (changeDeptList.length > 0){
        changeDeptList.forEach(dept => {
          const syzhejiuDept = shangyueZhejiuDeptList.filter(zhejiu => zhejiu.cardUnique == item.cardUnique && zhejiu.deptUnique == dept.deptUnique)
          const depreciationDept: any = {}
          depreciationDept.uniqueCode = item.uniqueCode
          depreciationDept.manageCode = item.manageCode
          depreciationDept.cardUnique = item.cardUnique
          depreciationDept.iyear = year.value
          depreciationDept.imonth = formItems.value.month
          if (syzhejiuDept.length > 0) {//上月计提折旧有值
            if (flag) {//是最后一个月
              //本月 = 原值-累计折旧-净残值
              depreciationDept.zjBy = sub(sub(mul(change.yuanzhi, div(dept.proportion, 100)), syzhejiuDept[0].zjLj), mul(mul(change.yuanzhi, div(change.jingcanzhilv, 100)), div(dept.proportion, 100))).toFixed(5)
            } else {
              depreciationDept.zjBy = mul(yuezhejiue, div(dept.proportion, 100)).toFixed(5)
            }
            depreciationDept.zjLj = add(syzhejiuDept[0].zjLj, depreciationDept.zjBy).toFixed(5)
            if (year.value == year1) {
              depreciationDept.zjBn = add(syzhejiuDept[0].zjBn, depreciationDept.zjBy).toFixed(5)
            } else {
              depreciationDept.zjBn = depreciationDept.zjBy
            }
          } else {//上月计提折旧没值
            if (flag) {//是最后一个月
              //本月 = 原值-累计折旧-净残值
              depreciationDept.zjBy = mul(sub(sub(change.yuanzhi, change.ljzhejiu), mul(change.yuanzhi, div(change.jingcanzhilv, 100))), div(dept.proportion, 100)).toFixed(5)
              depreciationDept.zjLj = add(mul(change.ljzhejiu, div(dept.proportion, 100)), depreciationDept.zjBy).toFixed(5)
              depreciationDept.zjBn = depreciationDept.zjBy
            } else {
              depreciationDept.zjBy = mul(yuezhejiue, div(dept.proportion, 100)).toFixed(5)
              depreciationDept.zjLj = mul(add(change.ljzhejiu, yuezhejiue), div(dept.proportion, 100)).toFixed(5)
              depreciationDept.zjBn = mul(yuezhejiue, div(dept.proportion, 100)).toFixed(5)
            }
          }
          depreciationDept.gzlBy = null
          depreciationDept.gzlLj = null
          depreciationDept.deptUnique = dept.deptUnique
          depreciationDept.deptBl = dept.proportion
          depreciationDeptList.value.push(depreciationDept)
        })
      }
      //项目不为空
      if (changeProjectList.length > 0){
        changeProjectList.forEach(project => {
          const syzhejiuProject = shangyueZhejiuProjectList.filter(zhejiu => zhejiu.cardUnique == item.cardUnique && zhejiu.projectUnique == project.projectUnique)
          const depreciationProject: any = {}
          depreciationProject.uniqueCode = item.uniqueCode
          depreciationProject.manageCode = item.manageCode
          depreciationProject.cardUnique = item.cardUnique
          depreciationProject.iyear = year.value
          depreciationProject.imonth = formItems.value.month
          if (syzhejiuProject.length > 0) {//上月计提折旧有值
            if (flag) {//是最后一个月
              //本月 = 原值-累计折旧-净残值
              depreciationProject.zjBy = sub(sub(mul(change.yuanzhi, div(project.proportion, 100)), syzhejiuProject[0].zjLj), mul(mul(change.yuanzhi, div(change.jingcanzhilv, 100)), div(dept.proportion, 100))).toFixed(5)
            } else {
              depreciationProject.zjBy = mul(yuezhejiue, div(project.proportion, 100)).toFixed(5)
            }
            depreciationProject.zjLj = add(syzhejiuProject[0].zjLj, depreciationProject.zjBy).toFixed(5)
            if (year.value == year1) {
              depreciationProject.zjBn = add(syzhejiuProject[0].zjBn, depreciationProject.zjBy).toFixed(5)
            } else {
              depreciationProject.zjBn = depreciationProject.zjBy
            }
          } else {//上月计提折旧没值
            if (flag) {//是最后一个月
              //本月 = 原值-累计折旧-净残值
              depreciationProject.zjBy = mul(sub(sub(change.yuanzhi, change.ljzhejiu), mul(change.yuanzhi, div(change.jingcanzhilv, 100))), div(project.proportion, 100)).toFixed(5)
              depreciationProject.zjLj = add(mul(change.ljzhejiu, div(project.proportion, 100)), depreciationProject.zjBy).toFixed(5)
              depreciationProject.zjBn = depreciationProject.zjBy
            } else {
              depreciationProject.zjBy = mul(yuezhejiue, div(project.proportion, 100)).toFixed(5)
              depreciationProject.zjLj = mul(add(change.ljzhejiu, yuezhejiue), div(project.proportion, 100)).toFixed(5)
              depreciationProject.zjBn = mul(yuezhejiue, div(project.proportion, 100)).toFixed(5)
            }
          }
          depreciationProject.gzlBy = null
          depreciationProject.gzlLj = null
          depreciationProject.projectUnique = project.projectUnique
          depreciationProject.projectBl = project.proportion
          depreciationProjectList.value.push(depreciationProject)
        })
      }
    }
  }
