<template>
  <div class="nci-hr">
    ...
    <div class="nc-hrm-body" :style="{maxHeight: windowHeight+'px'}">
      <div><span>取消结账月份：</span><span>{{ currMonth }}月</span></div>
      <div>
        <Steps :current="stepValue" status="error">
          <Step title="开始工作检查" description=""/>
          <Step :title="stepValue < 2?'未完成':'已完成'" description="清理变动"/>
          <Step :title="stepValue < 3?'未完成':'已完成'" description="清理拆分"/>
          <Step :title="stepValue < 4?'未完成':'已完成'" description="清理盘点"/>
          <Step :title="stepValue < 5?'未完成':'已完成'" description="清理评估"/>
          <Step :title="stepValue < 6?'未完成':'已完成'" description="取消月末结账"/>
        </Steps>
      </div>
      <div>
        <Button type="primary" shape="round" size="large" v-if="!indicator"
                :disabled="stepValue != 5" @click="startBill">
          开始取消结账
        </Button>
...
import {
  ...
  findByAuthAssetsByUniqueCode, findZiChanUnPeriodByCode
...
async function selectChange(v) {
  let dbE = await findZiChanUnPeriodByCode({code: v, iyear: dynamicYear.value})
  if (null != dbE && dbE.isSettle == '1') {
...
const startBill = async () => {
  tipText.value = '月末结账'
  indicator.value = true
  let e = await closeBill({id: currId.value, value: '0'})
  setTimeout(async () => {
    indicator.value = false
    if (null != e) {
