    ...
    <div class="app-container-top">
      <div class="app-container-top-one">
        ...
        <div class="act-one-d-title">
          <span class="acto-d-title-span">资产盘点列表</span>
        </div>
...
<!--          <span>管理代码：</span>
          <Select class="acttdrd-search-select" v-model:value="managementCode" style="width: 200px;"
                  placeholder="FA" @change="selectChange">
            <template #suffixIcon>
              <SearchOutlined/>
            </template>
    ...
    <div class="app-container-bottom" :style="{height: (windowHeight+60)+'px',marginTop: '5px'}">
      ...
      <div style="display: inline-block;height: 100%; float: left;margin-left: 5px;"
        ...
        >
          <template #pdId="{record,index }">
          <span class="tableUStyle"
                @click="toAddPage(record)">{{ record.pdId }}</span>&ensp;
          </template>
          <template #pdMethod="{record,index }">
            {{ pandiabTypeList[parseInt(record.pdMethod)] }}
          </template>
          <template #pdRange="{record,index }">
            {{ getCurrRangeText(record.pdMethod, record.pdRange) }}
          </template>
          <template #pdDept="{record,index }">
            {{ formatUniqueCodeDept(record.pdDept) }}
          </template>
          <template #pdUsername="{record,index }">
            {{ formatUniqueCodeUser(record.pdUsername) }}
          </template>
          <template #pdjdDept="{record,index }">
            {{ formatUniqueCodeDept(record.pdjdDept) }}
          </template>
          <template #pdjdUsername="{record,index }">
            {{ formatUniqueCodeUser(record.pdjdUsername) }}
          </template>
        </BasicTable>
      </div>
    </div>
...
const CrudApi = reactive({
  columns: [
    ...
    },
    {
      ...
      title: '盘点方式',
      dataIndex: 'pdMethod',
      slots: {customRender: 'pdMethod'}
    }, {
      title: '盘点范围',
      dataIndex: 'pdRange',
      slots: {customRender: 'pdRange'}
    }, {
      title: '盘点部门',
      dataIndex: 'pdDept',
      slots: {customRender: 'pdDept'}
    }, {
      title: '盘点人',
      dataIndex: 'pdUsername',
      slots: {customRender: 'pdUsername'}
    }, {
      title: '监督盘点部门',
      dataIndex: 'pdjdDept',
      slots: {customRender: 'pdjdDept'}
    }, {
      title: '监督盘点人',
      dataIndex: 'pdjdUsername',
      slots: {customRender: 'pdjdUsername'}
    }, {
      title: '制单人',
      dataIndex: 'maker',
      slots: {customRender: 'maker'}
    }, {
      title: '审核人',
...
const startDel = async () => {
  if (tableSelectedRowKeys.value.length == 0) {
    ...
    createConfirm({
      ...
      onOk: async () => {
        await useRouteApi(delPanDing, {schemaName: manipulateDbName.value})({id: tableSelectedRowKeys.value[0]})
...
const startReview = async () => {
  if (tableSelectedRowKeys.value.length == 0) {
    ...
    if (!hasBlank(a) && !hasBlank(tableSelectedRowKeys.value[0])) {
      await useRouteApi(reviewPanDing, {schemaName: manipulateDbName.value})({
...
import {delPanDing, findFaPanDianListByCondition, reviewPanDing} from "/@/api/fa/fa-pandian";
import {
