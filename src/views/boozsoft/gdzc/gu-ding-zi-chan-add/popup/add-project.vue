        ...
        <table class="layui-table" id="tableId" style="height: calc( 100% - 20px );margin-top: 10px;background-color:#f2f4f6;">
          ...
          <tbody>
            <tr class="odd" v-for="(row,rowIndex) in formItems">
              <td style="width: 50%;">
                <Select v-model:value="row.projectUnique" @change="changeProject(row,rowIndex)" style="width: 100%;">
...
const [register, {closeModal}] = useModalInner((data) => {
  projectList.value = data.data.projectList
  if (data.data.changeProjectList.length>0) {
    formItems.value = data.data.changeProjectList
  } else {
    formItems.value = [{
      uniqueCode: '',
      manageCode: '',
...
async function handleOk() {
  ...
  for (let i = 0; i < formItems.value.length; i++) {
    ...
    if (item.projectUnique==null || item.projectUnique==''){
      createErrorModal({
        iconType: 'warning',
        title: '提示',
        content: '第'+(i+1)+'行项目为空，不能进行保存！'
      })
      return false
    }
...
//判读部门是否重复
function changeProject(row,rowIndex){
  formItems.value.forEach((item,index)=>{
    if (rowIndex!=index){
      if (row.projectUnique==item.projectUnique){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '项目已存在，请重新选择项目！'
        })
        row.projectUnique=''
      }
    }
