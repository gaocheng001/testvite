...
async function findAllXySource() {
  let list = await useRouteApi(findAllByBianDongCcodeLike,{schemaName: schemaName})(ccodeV.value)
  setTableData(list)
  tableAll.value = list;
}
...
function goRuter(data) {
  if (!hasBlank(data.ccode)){
    router.push({name: 'StockCaiGouDHBD',params: {ccode:data.ccode,database:schemaName.value}});
    handleOk()
  }
}
