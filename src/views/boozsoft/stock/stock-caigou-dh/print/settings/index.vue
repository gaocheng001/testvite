...
import {findStockPrintAll, findStockPrintAllByCcode} from "/@/api/stockPrint";

const treeDatas = ref(printSettingTreeDatas)
...
function toTreeData(aaa){
  ...
  aaa.forEach(it=>{
    ...
    newArr[it.printModel].children.push({
      title: getTypeName(it.cstyle)+' '+it.printModel,
      key: it.id,
      icon: 'home|svg',
      target:it
...
function goPrintBody(typeNmae){

}
onMounted(async ()=>{
  const result=await findStockPrintAll()
  const stockPrint=result.stockPrint.map(it=>{
    return {
      title: getTypeName(it.cstyle)+' '+it.printModel,
      key: it.id,
      icon: 'home|svg',
  ...
  const newArr=toTreeData(result.stockPrint)
