...
const oldstockClass = ref('');
const oldstockCclassName = ref('');
...
const [register, { closeModal }] = useModalInner( async (data) => {
  ...
  formItems.value.uniqueStockclass = data.data.uniqueStockclass,
  formItems.value.stockClass = data.data.stockClass,
  formItems.value.stockClassGrade = data.data.stockClassGrade,
  formItems.value.parentId = data.data.parentId=='0'?'':data.data.parentId,
  formItems.value.stockCclassName = data.data.stockCclassName,
  formItems.value.stockBend = data.data.stockBend,
  formItems.value.flag = data.data.flag,
  formItems.value.stockGradeCode = data.data.stockGradeCode;

  oldstockClass.value=data.data.stockClass
  oldstockCclassName.value=data.data.stockCclassName
  if(formItems.value.cusClass==='9999'){inputDisabled.value=true}
});


async function handleOk() {
  let countStockCLass=await useRouteApi(countByStockClass,{schemaName: database})(formItems.value.parentId);
  if(countStockCLass>0){
    return createErrorPop('上级分类已有存货档案,不能增加下级！');
  }
  if(oldstockCclassName.value!==formItems.value.stockCclassName){
  ...
  if(oldstockClass.value!==formItems.value.stockClass){
