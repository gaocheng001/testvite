...
} from "/@/views/boozsoft/stock/stock-caigou-fapiao/component/DynamicForm";
...
const onCheckChange = (record) => {
  return {disabled: (record.label === '单据日期' || record.label === '单据编号' || record.label === '供应商' || record.label === '仓库')}
}
...
const [register, {closeModal, setModalProps}] = useModalInner(async (o) => {
  ...
  // 加载默认数据
  dynamicColumnDataCopy.value = JsonTool.parseProxy(list.filter(it => it.label != '单据日期' && it.label != '单据编号' && it.label != '供应商'))
  setTableData(list)
...
async function handleOk() {
  let list = getDataSource().filter(it => !hasBlank(it.field) && !hasBlank(it.label) && !hasBlank(it.component)).filter(it => it.label != '单据日期' && it.label != '单据编号' && it.label != '供应商')
