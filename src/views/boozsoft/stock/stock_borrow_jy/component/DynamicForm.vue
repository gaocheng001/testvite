...
<!--            <SearchOutlined @click="exec(field)"/>-->
      ...
      <template #unitType="{ model, field }">
        <Select placeholder="其他/客户/供应商/员工/部门/项目" v-model:value="model[field]" ref="cvencodeRef" :show-search="true"
                :filter-option="filterOption"  @change="model['unitValue']='',resetSchemas()"
                :options="[{value: 'etc',label: '其他'},{value: 'cust',label: '客户'},{value: 'supplier',label: '供应商'},{value: 'user',label: '员工'},{value: 'dept',label: '部门'},{value: 'project',label: '项目'}]">
      ...
      <template #unitValue="{ model, field }">
        <Select v-if="model['unitType'] != 'etc'" placeholder="请选择" v-model:value="model[field]" ref="cvencodeJsRef" :show-search="true"
        ...
        <Input placeholder="输入"  v-else v-model:value="model[field]" />
      </template>
...
function createItem(it) {
  return {
    ...
    slot: it['component'] == 'Select' || it['field'] == 'feiyongJe' ? it['field'] : null,
    show: it['isShow']
  }
}
