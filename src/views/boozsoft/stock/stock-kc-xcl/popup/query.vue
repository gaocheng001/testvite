    ...
    <div style="display: inline-flex;justify-content: space-between;width: 100%;">
      <div style="width: calc(100% - 150px);height: 100%;">
        <div style="text-align: center;padding: 10px 0 5px;">
          ...
          <span style="line-height:30px;font-size: 24px;color: #0096c7;font-weight: bold;">&ensp;现存量查询</span>
        </div>
        <div
              ...
              <div style="margin-left: 14px;">
                <ul>
                  <li style="margin: 2% 0px;">
                    <label>截止日期：</label>
                    <a-date-picker :allowClear="false" placeholder="" style="width: 320px;text-align: center;" v-model:value="endDate" @change="verifyDate"/>
                  </li>
                  <li style="margin: 2% 0px;">
...
//************************************* new ***************************
const endDate: any = ref(dayjs(useCompanyOperateStoreWidthOut().getLoginDate));
const ckId: any = ref('');
...
function handleOk() {
  let map={
    endDate:hasBlank(endDate.value)?'':transformTimestamp(endDate.value),
...
async function verifyDate() {
  ...
  let tmp=period.filter(a=>a==transformTimestamp(endDate.value).split('-')[0])
  if(tmp.length==0){
     endDate.value=period[period.length-1]+'12-31'
  }
}
</script>
