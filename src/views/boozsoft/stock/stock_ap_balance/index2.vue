  ...
  <div>
    <div class="app-container">
      <div class="app-container-head">
        <div class="container-head-title" style="width: 57%;">
          <AccountPicker  theme="one" @reloadTable="dynamicAdReload"/>
          <span style="font-size: 24px;color: #0096c7;font-weight: bold;float: right;margin-top: -27px;">应付款期初余额</span>
        </div>
        ...
        <div class="bg-white m-4 mr-0 overflow-hidden" style="margin-top: -0.5px;margin-left: -5px;float: left;width: 20%;">
          <div style="width: 100%; height: 32px;background-color: #d9d9d9;padding-top:5px;padding-left:10px;font-weight: bold;">
            ...
            <div style="float: right;">
              <a style="color: black;font-size: 14px;" @click="findArCustAll"><reloadOutlined /></a>&emsp;
              ...
              <a style="color: black;font-size: 14px;" @click="custModalShow"><plusOutlined /></a>&emsp;
              <a style="color: black;font-size: 14px;" @click="delArBalance"><minusOutlined /></a>&emsp;
            </div>
        ...
        <div style="width: 80%;float: right;margin-left: 5px;">
          <BasicTable ref="tableRef" @register="registerTable" :dataSource="tableData" :scroll="{ x: totalColumnWidth,y: windowHeight }" :class="'a-table-font-size-12'" @selection-change="selectionChange">
            ...
            <template #arStyle="{ record }">
              {{ record.arStyle === 'XSFP' ? '销售发票' : record.arStyle === 'YSK'?'应付款':record.arStyle === 'YUSK'?'预付款':'应付票据' }}
            </template>
            <template #arDir="{ record }">
              {{ record.arDir === '1' ? '预付' : '应付' }}
            </template>
    ...
    <SupModalPop @throwData="custThrowData" @register="registerCustModalPopPage" />
  </div>
</template>
...
import {
  auditStockArBalance,
  countByCustIdAndIyearAndBcheck,
  delStockArBalance,
...
} from "/@/api/record/stock/stock_ap_balance";
import {useUserStoreWidthOut} from '/@/store/modules/user';
import {useModal} from "/@/components/Modal";
import Add from '/@/views/boozsoft/stock/stock_ap_balance/popup/save.vue';
import Edit from '/@/views/boozsoft/stock/stock_ap_balance/popup/edit.vue';
...
import {
  ...
  stockArCustFindAll,
  stockArCustSave
} from "/@/api/record/stock/stock_ap_sup";
import {assemblyDynamicColumn, initDynamics} from "/@/views/boozsoft/stock/stock_ap_balance/data";
...
const lanMuData = {'accId':'','menuName':'应付款期初余额','type':'账套',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
...
const custDataChange:any = ref('')
     ...
     let taskData= await useRouteApi(getByStockArBalanceTask, { schemaName: database })(iyear.value)
      if(taskData==''){
        ...
        if(taskData.caozuoUnique!==useUserStoreWidthOut().getUserInfo.username){
          return createWarningModal({ content: taskData.caozuoUnique+'正在编辑库应付款期初余额,不能同时进行操作！' });
        }else{
  ...
  async function balanceTaskDel() {
    await useRouteApi(stockArBalanceTaskDelByUserName, { schemaName: database })({iyear:iyear.value,userName:useUserStoreWidthOut().getUserInfo.username,functionModule:'应收款期初余额',method:'编辑'})
  }
  async function balanceTaskSave() {
    await useRouteApi(stockArBalanceTaskSave, { schemaName: database })({iyear:iyear.value,userName:useUserStoreWidthOut().getUserInfo.username,functionModule:'应收款期初余额',method:'编辑'})
  }
...
async function delArBalance() {
  ...
  for (let i = 0; i < checkedKeys.value.length; i++) {
    ...
    if(sum>0){
      return createWarningModal({ content: '供应商【'+treeData.value[0].children.filter(a=>a.key==checkedKeys.value[i])[0].custName+'】期初有已审核,不能删除！' });
    }else{
...
function addFun() {
  ...
  if(custDataChange.value==''){
    return createWarningModal({ content: '请选择供应商档案！' });
  }
  openSavePage(true,{
    database:database.value,
    iyear:iyear.value,
