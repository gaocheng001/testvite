      ...
      <div class="content-head">
        <Row style="width: 100%;">
          ...
          <Col style="text-align: right;" :span="6">
            <Button size="small" v-if="kcCgrkCheck=='0'" @click="reset('keyong')">按可用量出库</Button>&emsp;
...
const [register, {closeModal, setModalProps}] = useModalInner(async (o) => {
  ...
  let c:any=getColumns()
    ...
    if(dynamicTenant.value.target?.kcCgrkCheck=='0'){
...
async function reloadPage() {
  ...
  tableSelectedRowObjs.value = []
   ...
   let list= await useRouteApi(batchSelectorXCLList, {schemaName: schemaName.value})({queryType:dynamicTenant.value.target?.kcCgrkCheck=='0'?'keyong':'xcl',unitId:rowInfo.value.cgUnitId,rkBcheck:rkBcheck.value,ckBcheck:ckBcheck.value,bdocumStyle:bdocumStyle.value,iyear:iyear.value,cinvode:rowInfo.value.cinvode,cwhcode:rowInfo.value.cwhcode})
    xclLength.value=list.length
    list.forEach(xc=>{
      let c = rowInfo.value['cunitid']
      let x = rowInfo.value['cgUnitId']
      let bNum =  parseFloat(kcCgrkCheck.value=='1'?xc['xcl']:xc['keyong'])
