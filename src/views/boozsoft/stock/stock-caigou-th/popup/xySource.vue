  ...
  <BasicModal width="700px"
    ...
    <template #title>
      <div style="display: flex;margin-left: 10px;margin-top: 5px;" class="vben-basic-title">
        ...
        <span style="line-height: 29px;font-size: 20px;margin-left: 10px;color: rgb(21, 142, 184)">采购退货单下游单据</span>
      </div>
    </template>
      ...
      <div class="content-body" style="margin-top: 5px;">
        ...
        >
          <template #xyBillStyle="{ record }">
            {{ record.xyBillStyle=='CGRKD'?'采购入库单':record.xyBillStyle=='CGJSD'?'采购结算单':record.xyBillStyle=='JZPZ'?'记账凭证':'' }}
          </template>
...
async function goRuter(data) {
  if(pageParameter.xytype=='CGRKD'){
    ...
    setTimeout(()=>{
      router.push({path: 'kc-cgDepot',query: {ccode:data.xyccode,bdocumStyle:'1',co: databaseCo.value}});
    },1000)
  }else if(pageParameter.xytype=='CGFP'){
    await closeToFullPaths('/cg-bill')
    setTimeout(()=>{
      router.push({path: 'cg-bill',query: {ccode:data.xyccode,bdocumStyle:'1',co: databaseCo.value}});
    },1000)
  }else if(pageParameter.xytype=='CGDHD'){
    await closeToFullPaths('/cg-return')
    setTimeout(()=>{
      router.push({path: 'cg-return',query: {ccode:data.xyccode,bdocumStyle:'1',co: databaseCo.value}});
    },1000)
  }else if(pageParameter.xytype=='CGJSD'){
    await closeToFullPaths('/cg-jiesuan')
    setTimeout(()=>{
      router.push({path: 'cg-jiesuan',query: {ccode:data.xyccode,bdocumStyle:'1',co: databaseCo.value}});
    },1000)
  }
  handleOk()
}
