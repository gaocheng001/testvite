  ...
  <BasicModal width="700px"
    ...
    <template #title>
      <div style="display: flex;margin-left: 10px;margin-top: 5px;" class="vben-basic-title">
        ...
        <span style="line-height: 29px;font-size: 20px;margin-left: 10px;color: rgb(21, 142, 184)">其他入库单来源单据</span>
      </div>
    </template>
...
async function goRuter(data) {
  if(pageParameter.xytype=='CGRKD'){
    ...
    setTimeout(()=>{
      router.push({path: 'cg-return',query: {ccode:data.xyccode,co: databaseCo.value}});
    },1000)
  }else if(pageParameter.xytype=='CGDD'){
    await closeToFullPaths('/cg-dingdan')
    setTimeout(()=>{
      router.push({path: 'cg-dingdan',query: {ccode:data.xyccode,co: databaseCo.value}});
    },1000)
  }
  handleOk()
}
