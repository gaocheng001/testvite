  ...
  <BasicModal width="925px"
    ...
    <template #title>
      <div style="height: 30px;width: 100%;background-color: #5f375c;color: white;line-height: 30px;text-align: left;">
        ...
        <span style="font-size: 14px">栏目管理</span>
      </div>
    </template>
    <div class="nc-query-open-content">
      <div class="content-head">
        <span style="font-size: 20px;font-weight: bold;color: white;float: left;">表头栏目显示设置</span>
        <Button @click="addRow">
          增行
        </Button>&nbsp;&nbsp;
        <Button @click="delRow">
      ...
      <div class="content-body">
        ...
        >
          <template #columnType="{ record }">
            <Tag color="#1890ff" v-if="record.columnType == '1'">系统</Tag>
...
import {CheckOutlined, EditOutlined,AppstoreOutlined} from '@ant-design/icons-vue';
...
const onCheckChange = (record) => {
  return {disabled: (record.label === '入库日期' || record.label === '入库单号' || record.label === '供应商' || record.label === '仓库')}
}
...
const [register, {closeModal, setModalProps}] = useModalInner(async (o) => {
  ...
  lanMuData.menuName = o.title+'表头栏目'
  let list = await GenerateDynamicColumn(schemaName.value,o.title,o.rows)
  // 加载默认数据
  dynamicColumnDataCopy.value = JsonTool.parseProxy(list.filter(it => it.label != '入库日期' && it.label != '入库单号' && it.label != '供应商'))
  setTableData(list)
