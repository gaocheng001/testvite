  ...
  <BasicModal width="700px"
    ...
    <template #title>
      <div style="display: flex;margin-left: 10px;margin-top: 5px;" class="vben-basic-title">
        ...
        <span style="line-height: 29px;font-size: 20px;margin-left: 10px;color: rgb(21, 142, 184)">{{titlePrefix}}来源单据</span>
      </div>
    </template>
...
import {findByLyOutDataSourrce} from "/@/api/record/stock/stock-xy-source";
...
const resetTool =  () => {
  if (queryType.value == 'XHD'){
    ...
    if (isRed.value){
      groupList.value =[{value: 'XHD',title:'销货单',path:'/xs-arrive'}]
    }else {
      groupList.value =[{value: 'XSDD',title:'销售订单',path:''},{value: 'XSCKD',title:'销售出库单',path: '/kc-xsDepot'}]
    }
    queryV.value =  groupList.value[0].value
  }else if(queryType.value == 'XSCKD'){
    titlePrefix.value = (isRed.value?'红字':'')+'销售出库单'
    groupList.value =[{value: 'XSDD',title:'销售订单',path:''},{value: 'XHD',title:isRed.value?'退货单':'销货单',path:isRed.value?'/xs-return':'/xs-arrive'}]
    if (isRed.value)groupList.value = groupList.value.filter(it=>it.value != 'XSDD')
    queryV.value =  groupList.value[0].value
  }else if(queryType.value == 'XSFP'){
    titlePrefix.value = (isRed.value?'红字':'')+'销售发票'
    groupList.value =[{value: 'XHD',title:isRed.value?'退货单':'销货单',path:isRed.value?'/xs-return':'/xs-arrive'},{value: 'XSDD',title:'销售订单',path:''},{value: 'XSCKD',title:(isRed.value?'红字':'')+'销售出库单',path:'/kc-xsDepot'}]
    queryV.value =  groupList.value[0].value
  }else {
    groupList.value =[]
    titlePrefix.value =''
...
async function findAllXySource() {
  let list = await useRouteApi(findByLyOutDataSourrce,{schemaName: schemaName})({billStyle:queryType.value,code:ccodeV.value,type: queryV.value})
  setTableData(list)
}
