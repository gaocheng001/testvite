      ...
      <div class="content-body" style="margin-top: 5px;">
        ...
        >
          <template #xyBillStyle="{ record }">
            {{ record.xyBillStyle=='CGRKD'?'采购入库单':record.xyBillStyle=='CGFP'?'采购发票':record.xyBillStyle=='CGJSD'?'采购结算单':record.xyBillStyle=='CGDHD'?'采购到货单':'' }}
          </template>
...
const [register, {closeModal, setModalProps}] = useModalInner(async (o) => {
  ...
  let dataBaseInfo=await findByStockAccId(o.dynamicTenantId.substring(0,o.dynamicTenantId.length-5))
...
async function goRuter(data) {
  if(pageParameter.xytype=='CGRKD'){
    ...
    setTimeout(()=>{
      router.push({path: 'kc-cgDepot',query: {ccode:data.xyccode,bdocumStyle:data.bdocumStyle,co: databaseCo.value}});
    },1000)
