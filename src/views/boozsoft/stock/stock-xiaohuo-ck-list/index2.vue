<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 24px;color: #0096c7;">销售出库单列表</b></div>
...
const operateBefore = async (rows) => {
  // 检查操作单据是否锁定
  let taskData= await useRouteApi(getByStockBalanceBatchTask, { schemaName: dynamicTenantId })({iyear:strDate.value,name:'销售出库单',method:'修改,审核,删除,整理现存量',recordNum: [...new Set(rows.map(it => it.ccode))].join()})
  if(taskData!=''){
...
async function toRouter(data,type) {
  ...
  await closeToFullPaths('/kc-xsDepot')
  setTimeout(()=>{
    router.push({path: '/kc-xsDepot',query: {type:type,ccode:data.ccode,co: dynamicTenant.value.coCode}});
  },1000)
}
