    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">存货价格表</span><br/><br/>
            <span>模板样式：</span>
            <Select
    ...
    <template #footer>
      <div style="height: 35px">
        <div style="float: right">
          ...
          <a-button @click="handleOk(true)" type="primary" :loading="uploading" :disabled="flg">开始导入
          </a-button>
        </div>
      </div>
    </template>
...
const tablelist:any = ref(['存货编码','存货名称','规格型号','主计量','协议价','零售价','最低价','最高价','一级客户价','二级客户价','三级客户价','四级客户价','五级客户价','六级客户价','七级客户价','八级客户价','主计量'])
const flg:any = ref(true)
function loadDataSuccess(excelDataList) {
  console.log(excelDataList);
  console.log(excelDataList[0].results);
  excelDataList[0].header.forEach(v=>{
    if(tablelist.value.indexOf(v) == -1){
      createErrorModal({
        iconType: 'warning',
        title: '提示',
        content: v+',非法表头,不能进行信息导入'
      })
      return false
    }
  })
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      ...
      if(item1.stockNum != null){
        item1.stockName = item['存货名称']
        item1.memberprice = item['协议价']
        item1.regularPrice = item['零售价']
        item1.minPrice = item['最低价']
        item1.maxPrice = item['最高价']
        item1.invscost1 = item['一级客户价']
        item1.invscost2 = item['二级客户价']
        item1.invscost3 = item['三级客户价']
        item1.invscost4 = item['四级客户价']
        item1.invscost5 = item['五级客户价']
        item1.invscost6 = item['六级客户价']
        item1.invscost7 = item['七级客户价']
        item1.invscost8 = item['八级客户价']
