      ...
      <div class="content-body">
        ...
        >
          <template #isumFapiao="{ record }">
            {{ record.isumFapiao>0?parseFloat(record.isumFapiao).toFixed(2):'' }}
          </template>
...
const busTypeList = {'XHD':'销货单','XSCKD':'销售出库单','XSDD':'销售订单'}
const tableLoading = ref(false)
const iyear = ref('')
const startYm = ref('')
const querenFpYs = ref(false)
const database = ref('')
...
const [registerTable1, { deleteSelectRowByKey:deleteSelectRowByKey1,setTableData:setTableData1,getSelectRows:getSelectRows1,getDataSource:getDataSource1}] = useTable({
  columns: [
    ...
    },
    {
      ...
      width: '120px',
      align: 'right',ellipsis: true,slots: {customRender: 'isumFapiao'}
    },{
      title: '本次使用',
...
const [register, {closeModal, setModalProps}] = useModalInner(async (data) => {
  ...
  pageParameter.btype = data.btype
  startYm.value = data.dynamicTenant?.target?.startDate
  querenFpYs.value = data.dynamicTenant?.target?.arSourceFlag == '1'
  nextTick(async ()=>{
...
async function findByTable(){
  ...
  let map={
    ...
    bstype: pageParameter.btype,billType: 'XSFP',qrfp:  querenFpYs.value?'1':'0'
  }
...
const onSelectChange = async (k, o) => {
  ...
  if(tempObj.length>0){
    ...
    stockWarehList.forEach(async (list)=>{
      ...
      let isumFapiao=hasBlank(list.isumFapiao)?0:parseFloat(list.isumFapiao)

      let keyong=baseQuantity-isumFapiao!=0?baseQuantity-isumFapiao:baseQuantity
      list.keyong=keyong
      list.newBaseQuantity=baseQuantity-isumFapiao!=0?baseQuantity-isumFapiao:null
    })
    let temp:any=[]
...
async function handleOk() {
  ...
  let list:any=tableSelectedRowObjs1.value.filter(a=>!hasBlank(a.id) && (!pageParameter.type == 'XSCKD'?true:parseFloat(a.price) != 0))
  list.forEach(a=>{
    a.id=null
