...
<!--   <div><textarea>供应商应付明细表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">供应商应付明细表</b>
  ...
  if (cvencode.value!=null && cvencode.value!='') {
    ...
    if (res>0) {
      tableDataAll.value.push(...res.map(item => {
        ...
        let yue = 0
        res2.forEach(item1 => {
          yue = add(yue, sub(item1.isum, item1.hxIsum == null ? '0' : item1.hxIsum))
        })
...
function openPage(data){
  ...
  } else if (data.billStyle=='CGFP') {
    router.push({path: 'cg-bill',query: {type:'info',ccode:data.ccode}});
  }
}
...
const excelData = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '供应商应付明细表_'+pageParameter.companyName)
}
