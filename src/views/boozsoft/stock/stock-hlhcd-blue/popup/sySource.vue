  ...
  <BasicModal width="700px"
    ...
    <template #title>
      <div style="display: flex;margin-left: 10px;margin-top: 5px;" class="vben-basic-title">
        ...
        <span style="line-height: 29px;font-size: 20px;margin-left: 10px;color: rgb(21, 142, 184)">采购入库单来源单据</span>
      </div>
    </template>
      ...
      <div class="content-body" style="margin-top: 5px;">
        ...
        >
          <template #billStyle="{ record }">
            {{ record.billStyle=='CGFP'?'采购发票':record.billStyle=='CGDHD'?'采购到货单':record.billStyle=='CGDD'?'采购订单':'' }}
          </template>
...
async function goRuter(data) {
  if(data.billStyle=='CGFP'){
    ...
    setTimeout(()=>{
      router.push({path: 'cg-arrive',query: {ccode:data.ccode,co: databaseCo.value}});
    },1000)
