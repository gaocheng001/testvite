<template>
  ...
  >
    ...
    <div class="nc-open-content" style="height: 100%">
      <SecondaryTitle :iconType="'query'">销售统计表</SecondaryTitle>
      <div class="open-content-up">
        ...
        <Tabs type="card" v-model:activeKey="defaultTabsKey" tabPosition="left" style="width: 20%;float: left;">
          <TabPane key="1" tab="存货销售统计表"></TabPane>
          <TabPane key="2" tab="客户销售统计表"></TabPane>
          <TabPane key="3" tab="业务员销售统计表"></TabPane>
          <TabPane key="4" tab="地区销售统计表"></TabPane>
        </Tabs>
...
<!--                <div style="width: 50%;float: left;">
                ...
                <template v-if="defaultTabsKey != '1'">
                <div style="width: 50%;float: left;">
                  <span class="right_span">是否赠品：</span>
                  <a-select
                    :allowClear="true"
                    show-searchplaceholder=""
                    style="width: 180px"
                    v-model:value="queryModel.variable.isGive">
                    <a-select-option value="">全部</a-select-option>
                ...
                <template v-if="defaultTabsKey == '2'">
                <div style="width: 50%;float: left;">
                ...
                <template v-if="defaultTabsKey == '3'">
                  <div style="width: 50%;float: left;" >
                ...
                <template v-if="defaultTabsKey == '4'">
                  <div style="width: 50%;float: left;" >
                    <span class="right_span">国家（地区）：</span>
                    <a-select
                      :allowClear="true"
                      :options="dataMap['country']"
                      v-model:value="queryModel.variable.country"
                      placeholder="国家/地区"
                      style="width: 180px"
                    ></a-select>
                  </div>
                  ...
                  <div style="width: 50%;float: left;" >
                    <span class="right_span">区（县）：</span>
                    <a-select
                      :allowClear="true"
                      :options="countySeatOptions"
                      v-model:value="countySeat"
                      placeholder="区、县"
                      style="width: 180px"
                    ></a-select>
                  </div>
...
const dynamicTenantTarget = ref(null)
let count = 0
const codeChange = async (value) => {
  ++count
  if (count == 2) return false; // 防止重复打开
  dynamicTenantTarget.value = value?.target
  // 赋值默认参数
...
async function reLifeEtcList(dynamicTenant) {
  ...
  dataMap.value['country'] = (await findAllCountry()).items.map(it=>({value: it?.countryId,label: it?.namech}))
  countyOptions.value = JsonTool.parseProxy(dataMap.value['zone'])?.map(it=>{it.children.map(it2=>{it2.children = [];return it2});return it;})
}
...
async function handleOk() {
  ...
  if (defaultTabsKey.value == '4' && hasBlank(queryModel.variable.region)){
      message.error('按照地区查询,省市为必填项!');
      return false;
  }
  let ss = queryModel.variable.region
  if (ss.split(',').length==3)ss = ss.substring(0,ss.lastIndexOf(','))
  if (!hasBlank(countySeat.value)){
    queryModel.variable.region =  ss +','+countySeat.value
  }else {
    queryModel.variable.region = ss
  }
  ...
  queryModel.variable.xsckCheck = dynamicTenantTarget.value?.kcXsckCheck == '1'?'all':'1'
  modelLoadIng.value = false
...
const countySeat = ref('')
const countyOptions = ref([])
const countySeatOptions = ref([])
function zoneChange(a,b) {
  countySeat.value = ''
  countySeatOptions.value = JsonTool.parseProxy(dataMap.value['zone']).filter(it=>it.value == a[0])[0]?.children.filter(it=>it.value == a[1])[0]?.children
