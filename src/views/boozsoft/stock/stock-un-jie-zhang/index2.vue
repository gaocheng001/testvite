<template>
  <div class="nci-hr">
    ...
    <div class="nc-hr-middle">
      ...
      <div>
        <Button v-show="isClose" @click="startBill">
          取消结账
        </Button>
    ...
    <div class="nc-hrm-body" :style="{maxHeight: windowHeight+'px'}">
      <div>
        ...
        >
          <template #istock="{ record }">
            <Tag v-if="record.istock == '1'" color="green">已结账</Tag>
...
const yearChange = async () => {
  ...
  if (null == res || res.length == 0){
    ...
    setTableData(list)
    isClose.value = list.filter(it=>it.istock=='1').length>0?true:false
  }
}
const [registerTable, {
  reload,
...
const startBill = async () => {
  if (tableSelectedRowObjs.value.length != 1){
    message.success('请选中一条在进行取消结账操作！')
  }else if( tableSelectedRowObjs.value[0].istock !='1'){
