  ...
  <BasicModal width="950px"
    ...
    <template #title>
      <div style="display: flex;margin-left: 10px;margin-top: 5px;" class="vben-basic-title">
        ...
        <span style="line-height: 29px;font-size: 20px;margin-left: 10px;color: rgb(21, 142, 184)">采购入库单生单参照</span>
      </div>
    </template>
...
async function findByTable(){
  ...
  let ddata:any=data.filter(a=>a.rukuStatus!=='1'&&a.bdocumStyle=='0')
  // 去除：到货来源单据是否入库单
...
async function handleOk() {
  ...
  list.forEach(async (a)=>{
    a.isGive=a.isGive ? '1' : '0'
    a.isBatch=a.isBatch ? '1' : '0'
    a.oldBaseQuantity=(hasBlank(a.baseQuantity)?0:a.baseQuantity)-(hasBlank(a.isumRuku)?0:a.isumRuku)
