...
} from "/@/views/boozsoft/stock/stock_other_add/component/DynamicForm";
...
async function handleOk() {
  let list = getDataSource().filter(it => !hasBlank(it.field) && !hasBlank(it.label) && !hasBlank(it.component)).filter(it => it.label != '单据日期' && it.label != '单据单号' && it.label != '供应商')
