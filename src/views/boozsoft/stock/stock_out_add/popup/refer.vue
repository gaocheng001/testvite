      ...
      <div class="content-body">
        ...
        >
          <template #isumTuiHuo="{ record }">
            {{ record.isumTuiHuo>0?parseFloat(record.isumTuiHuo).toFixed(2):'' }}
          </template>
...
const busTypeList = {'XHD':'销货单','QCXHD':'期初销货单','XSDD':'销售订单'}
...
const [registerTable1, { deleteSelectRowByKey:deleteSelectRowByKey1,setTableData:setTableData1,getSelectRows:getSelectRows1,getDataSource:getDataSource1}] = useTable({
  columns: [
    ...
    },
    {
      ...
      width: '120px',
      align: 'right',ellipsis: true,slots: {customRender: 'isumTuiHuo'}
    },{
      title: '本次使用',
...
async function findByTable(){
  ...
  let map={
    ...
    bstype: pageParameter.btype,billType: 'XSCKD'
  }
...
const onSelectChange = async (k, o) => {
  ...
  if(tempObj.length>0){
    ...
    stockWarehList.forEach( (list)=>{
      ...
      let isumTuiHuo=hasBlank(list.isumTuiHuo)?0:parseFloat(list.isumTuiHuo)

      let keyong=baseQuantity-isumTuiHuo
      list.keyong=keyong
