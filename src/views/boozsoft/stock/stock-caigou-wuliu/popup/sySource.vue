  ...
  <BasicModal width="700px"
    ...
    <template #title>
      <div style="display: flex;margin-left: 10px;margin-top: 5px;" class="vben-basic-title">
        ...
        <span style="line-height: 29px;font-size: 20px;margin-left: 10px;color: rgb(21, 142, 184)">销售送货物流单来源单据</span>
      </div>
    </template>
    <div class="nc-query-open-content">
      <div
        ...
        <div style="float: left;margin-right: 5px;">
          <span>
            <RadioGroup v-model:value="pageParameter.sytype" @change="findAllXySource">
        ...
        <div style="float: right;margin-right: 5px;">
          <button type="button" class="ant-btn ant-btn-me" @click="findAllXySource"><span>刷新</span>
          </button>
        </div>
      </div>
      <div class="content-body" style="margin-top: 5px;">
        ...
        >
          <template #xyBillStyle="{ record }">
            {{
              record.xyBillStyle == 'CGRKD' ? '采购入库单' : record.xyBillStyle == 'CGFP' ? '采购发票' : record.xyBillStyle == 'CGDHD' ? '采购到货单' : record.xyBillStyle == 'CGDD' ? '采购订单' : ''
            }}
          </template>
...
async function findAllXySource() {
  ...
  pageParameter.xyccode = xyccode.value
  let list = await useRouteApi(findBySyDataMainSourrceMap, {schemaName: schemaName})(pageParameter)
  let tableLength = 0
...
async function goRuter(data) {
  if(pageParameter.xytype=='CGRKD'){
    ...
    setTimeout(()=>{
      router.push({path: 'kc-cgDepot',query: {ccode:data.ccode,co: databaseCo.value}});
    },1000)
  }else if(pageParameter.xytype=='CGFP'){
    await closeToFullPaths('/cg-bill')
    setTimeout(()=>{
      router.push({path: 'cg-bill',query: {ccode:data.ccode,co: databaseCo.value}});
    },1000)
  }else if(pageParameter.xytype=='CGDHD'){
    await closeToFullPaths('/cg-return')
    setTimeout(()=>{
      router.push({path: 'cg-return',query: {ccode:data.ccode,co: databaseCo.value}});
    },1000)
  }else if(pageParameter.xytype=='CGDD'){
    await closeToFullPaths('/cg-dingdan')
    setTimeout(()=>{
      router.push({path: 'cg-dingdan',query: {ccode:data.ccode,co: databaseCo.value}});
    },1000)
  }
  handleOk()
}
