<template>
  ...
  >
    ...
    <div style="width: calc(100% - 265px); float: right;margin-top: 5px;">
      <BasicTable @register="registerTable" :scroll="{ y:400 }" :dataSource="tableData" :row-selection="{ type: multipleFlag?'checkbox':'radio',selectedRowKeys: selectedRowKeys, onChange: onSelectChange}" @row-dbClick="rowDbClick" :class="'a-table-font-size-12'">
...
async function findByStockByEnterTime() {
  await useRouteApi(findAll,{schemaName: database})(pageParameter)
  .then(async (data)=>{
