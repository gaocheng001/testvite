...
<!--   <div><textarea>取消结账的页面</textarea>复制内容快速定位页面</div>-->
    ...
    <div class="nc-hrm-body" :style="{maxHeight: windowHeight+'px'}">
      <div>
        ...
        >
          <template #iar="{ record }">
            <Tag v-if="record.iar == '1'" color="green">已结账</Tag>
...
import {savePeriod} from "/@/api/record/system/ar-ap-period";
...
const startBill = async () => {
  if (tableSelectedRowObjs.value.length != 1){
    ...
    let e = await savePeriod({id: currId.value, value: '0',user: userId})
    setTimeout(async () => {
      if (null != e) {
