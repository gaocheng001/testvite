          ...
          <Step>
            <template #title>
              <div class="ant-steps-item-title">
                ...
                <span :class="stepModel['0'].chgl.finish?'span-green':'span-red'">{{stepModel['0'].chgl.finish ?'存货结账':'存货结账'}}</span>
              </div>
            </template>
...
<!--          <Step :title="stepModel['1'].qcye.finish?'期初余额':'期初余额'">-->
          <Step>
            <template #title>
              <div class="ant-steps-item-title">
                <span style="color:red;">*</span>
                <span :class="stepModel['1'].qcye.finish?'span-green':'span-red'">{{stepModel['1'].qcye.finish ?'期初余额':'期初余额'}}</span>
              </div>
            </template>
            <template #description>
...
<!--          <Step :title="stepModel['2'].ysd.finish ?'应收单审核':'应收单审核'">-->
          <Step>
            <template #title>
              <div class="ant-steps-item-title">
                <span style="color:red;">*</span>
                <span :class="stepModel['2'].ysd.finish?'span-green':'span-red'">{{stepModel['2'].ysd.finish ?'应收单审核':'应收单审核'}}</span>
              </div>
            </template>
            <template #description>
              <div class="step-div" v-if="!stepModel['2'].ysd.finish">
                <span :class="stepModel['2'].ysd.finish?'span-green':'span-red'">{{stepModel['2'].ysd.number}}个应收单{{stepModel['2'].ysd.finish?'已':'未'}}审核</span><br/>
              </div>
            </template>
          </Step>
<!--          <Step :title="stepModel['3'].skd.finish ?'收款单审核':'收款单审核'">-->
          <Step>
            <template #title>
              <div class="ant-steps-item-title">
                <span style="color:red;">*</span>
                <span :class="stepModel['3'].skd.finish?'span-green':'span-red'">{{stepModel['3'].skd.finish ?'收款单审核':'收款单审核'}}</span>
              </div>
            </template>
            <template #description>
              <div class="step-div" v-if="!stepModel['3'].skd.finish">
                <span :class="stepModel['3'].skd.finish?'span-green':'span-red'">{{stepModel['3'].skd.number}}个收款单{{stepModel['3'].skd.finish?'已':'未'}}审核</span><br/>
              </div>
            </template>
          </Step>
<!--          <Step :title="stepModel['4'].skdhx.finish ?'收款单核销':'收款单核销'">-->
          <Step>
            <template #title>
              <div class="ant-steps-item-title">
<!--                <span style="color:red;">*</span>-->
                <span :class="stepModel['4'].skdhx.finish?'span-green':'span-red'">{{stepModel['4'].skdhx.finish ?'收款单核销':'收款单核销'}}</span>
              </div>
            </template>
            <template #description>
              <div class="step-div" v-if="!stepModel['4'].skdhx.finish">
                <span :class="stepModel['4'].skdhx.finish?'span-green':'span-red'">{{stepModel['4'].skdhx.number}}个收款单{{stepModel['4'].skdhx.finish?'已':'未'}}核销</span>
              </div>
            </template>
          </Step>
<!--          <Step :title="stepModel['5'].hzhc.finish ?'红字回冲单':'红字回冲单'">-->
          <Step>
            <template #title>
              <div class="ant-steps-item-title">
<!--                <span style="color:red;">*</span>-->
                <span :class="stepModel['5'].hzhc.finish?'span-green':'span-red'">{{stepModel['5'].hzhc.finish ?'红字回冲单':'红字回冲单'}}</span>
              </div>
            </template>
            <template #description>
...
<!--          <Step :title="stepModel['6'].pzzd.finish ?'凭证制单':'凭证制单'">-->
          <Step>
            <template #title>
              <div class="ant-steps-item-title">
<!--                <span style="color:red;">*</span>-->
                <span :class="stepModel['6'].pzzd.finish?'span-green':'span-red'">{{stepModel['6'].pzzd.finish ?'凭证制单':'凭证制单'}}</span>
              </div>
            </template>
            <template #description>
              <div class="step-div" v-if="!stepModel['6'].pzzd.finish">
                <span :class="stepModel['6'].pzzd.finish?'span-green':'span-red'">{{stepModel['6'].pzzd.number}}个应收及收款单据{{stepModel['6'].pzzd.finish?'已':'未'}}制单</span><br/>
              </div>
            </template>
          </Step>
...
import {
  ...
  findSaleousingByDateList, savePeriod
...
const startBill = async () => {
  ...
  if (isClose.value){ // 开始结账
    tipText.value = '月末结账'
    let e = await savePeriod({id: currId.value, value: '1',user: userId})
    setTimeout(async () => {
    ...
    for (let i = 0; i < res.length; i++) {
      ...
      if (item.stockMonth == month.value) {
        if (item.stockFlag == '1') {
          //锁定结账任务
          let taskData= await useRouteApi(findByIyearAndCaozuoModule, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoModule:'ar'})
          ...
          if(ymjz.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收款月末结账',method:'月末结账',caozuoModule:'ar'})
          } else {
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:ymjz[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收款月末结账',method:'月末结账',caozuoModule:'ar'})
          }
        }
      }
    }
    const res1 = await useRouteApi(findBeginBalanceByYearList,{schemaName: dynamicTenantId})({billStyle:'ar',year:dynamicYear.value})
    const qcye = res1.filter(item =>item.bcheck!='1')
    const res2 = await useRouteApi(findSaleousingByDateList, {schemaName: dynamicTenantId})({
      billStyle: 'YSD',
