    ...
    >
      <template #title>
        ...
        <span style="font-size: 24px;line-height:30px;">
          <SearchOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;手工核销
        </span>
        </div>
      </template>
...
async function reloadList(){
  ...
  if (apSourceFlag.value!='1') {
    qcysdList.value = qcList.filter(item => item.busStyle == 'YFD')
  } else {
    qcysdList.value = qcList.filter(item => item.arStyle == 'YFD')
  }
  ...
  if (apSourceFlag.value=='1') {
    tableData.value.push(...xhdList.value.filter(item => item.busStyle !='CGDHD' && item.bworkable=='1').map(item=>{
    ...
    if(apCheckFlag=='1'){
      tableData.value.push(...xhdList.value.filter(item => item.busStyle != 'CGFP' && item.bworkable=='1').map(item=>{
        item.hxStyle = item.billStyle
        item.hxCcode = item.ccode
        if (item.hxIsum!=null && item.hxIsum !='') {
          item.whxIsum = sub(item.isum, item.hxIsum)
      ...
      tableData.value.push(...xhdList.value.filter(item => item.busStyle != 'CGFP').map(item=>{
        item.hxStyle = item.billStyle
        item.hxCcode = item.ccode
        if (item.hxIsum!=null && item.hxIsum !='') {
          item.whxIsum = sub(item.isum, item.hxIsum)
