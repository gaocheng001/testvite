...
function formatBillStyle(billStyle){
  ...
  if (billStyle=='CGDHD'){
    ...
    str = '采购发票'
  } else if (billStyle=='QCCGDHD'){
    str = '期初采购到货单'
  } else if (billStyle=='QCYFD'){
    str = '期初应付单'
  } else if (billStyle=='QCCGFP'){
    str = '期初采购发票'
  } else if (billStyle=='FKD'){
    str = '付款单'
  } else if (billStyle=='QCFKD'){
    str = '期初付款单'
  }
  return str
}
...
async function reloadList(){
  ...
  qcysdList.value = qcList.filter(item => item.busStyle == 'FKD' && item.arIsumBenbi != 0)
