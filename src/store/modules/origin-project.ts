...
// @ts-ignore
import {findProClassTree} from "/@/api/record/system/origin-project-class";
import { ref } from 'vue';
...
export const useProjectStore = defineStore({
  ...
  actions:{
    ...
    async bb({treeData,psnList,classList,formItems,data}:any){
      ...
      treeData.value = []
      //选择人员
      const res2:any=await psnFindByFlag({originId:data.originId})
      psnList.value = res2
      //选择项目分类
      const res3:any=await findProClassTree(data.originId)
